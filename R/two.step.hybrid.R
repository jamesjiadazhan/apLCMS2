...
#' @param info A table with two columns. The first column is the file names, and the second column is the batch label of each file.
#' @param min.within.batch.prop.detect A feature needs to be present in at least this proportion of the files, for it to be initially detected as a feature for a batch. This parameter replaces the "min.exp" parameter in semi.sup().
#' @param min.within.batch.prop.report A feature needs to be present in at least this proportion of the files, in a proportion of batches controlled by "min.batch.prop", to be included in the final feature table. This parameter replaces the "min.exp" parameter in semi.sup().
#' @param min.batch.prop A feature needs to be present in at least this proportion of the batches, for it to be considered in the entire data.
...
#'
#' @details The function first conducts hybrid feature detection and alignment in each batch separately. Then a between-batch RT correction and feature alignment is conducted. Weak signal recovery is conducted at the single feature table level.
#'
