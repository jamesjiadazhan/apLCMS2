...
#'
#' @param eic.rec The matrix of data features from every EIC. Each row is an EIC. Each column is a data feature value.
#' @param known.mz The m/z values of the known metabolic features.
#' @param mass.matched An indicator vector. "1" means the corresponding EIC has an m/z matched to known features. The default is NA, in which case the matching is done inside this function.
#' @param to.use The maximum number of data features to use in the predictive models.
...
#' @param do.grp.reduce Whether to reduce the data features first by reducing each group of similar features into one.
#' @param remove.bottom The number of worst performing data features to remove before model building. If true, the removal is done based on single predictor ROC analysis.
#' @param max.fpr The threshold for selecting unmatched EICs. Each EIC is assigned an FPR value based on the final prediction model. Those with FPR smaller than this threshold will be selected. If a vector is provided, the first one will be used. But all FPR values will also be returned. So other functions will be able to make selections based on other threshold values.
#' @param min.tpr The threshold for selecting matched EICs. Each EIC is assigned an TPR value based on the final prediction model. Those with TPR larger than this threshold will be selected. If a vector is provided, the first one will be used. But all TPR values will also be returned. So other functions will be able to make selections based on other threshold values.
#'
...
#'
#' @return A list item is returned.
